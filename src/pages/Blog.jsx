  ...
  return (
    <section className="p-6 max-w-5xl mx-auto">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-2 mb-6">
        <input 
          className="border p-2 rounded break-words" 
          placeholder="Title" 
          value={form.title} 
        ...
        <AnimatePresence>
          {posts.map(p=>(
            <motion.li 
              ...
              animate={{ opacity:1 }} 
              exit={{ opacity:0 }} 
              className="border p-4 rounded flex justify-between items-start gap-2 flex-col md:flex-row"
            >
              <div className="min-w-0">
                <h3 className="font-semibold break-words">{p.title}</h3>
                <p className="text-gray-600 break-words">{p.content.slice(0,300)}...</p>
              </div>
